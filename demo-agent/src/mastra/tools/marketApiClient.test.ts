...
test("searchProducts calls /agent/search and returns compact matches", async () => {
  const urls: string[] = [];
  ...
  assert.equal(result.debugRef, 82);
  assert.equal(result.bestMatches[0]?.title, "Apple iPhone 15 128GB");
  assert.equal(result.bestMatches[0]?.score, 0.91);
  assert.equal(result.bestMatches[0]?.priceARS, 1234);
  assert.equal(result.bestMatches[0]?.priceUSD, null);
});

test("searchProducts passes the requested limit through unchanged", async () => {
  const urls: string[] = [];
  const fetcher: FetchLike = async (url) => {
    urls.push(String(url));
    return jsonResponse({
  ...
  assert.equal(urls[0], "http://api.test/agent/search?query=kindle+paperwhite&limit=1&mode=interactive");
});

test("getMatchingCandidates polls until V5 predictions arrive", async () => {
  let calls = 0;
  ...
  assert.equal(result.predictedCount, 1);
  assert.equal(result.candidates[0]?.modelDecision, "same");
});

test("getPriceHistory calls the run history endpoint", async () => {
  const urls: string[] = [];
  const fetcher: FetchLike = async (url) => {
    urls.push(String(url));
    return jsonResponse({
  ...
  assert.equal(urls[0], "http://api.test/agent/search/82/history");
  assert.equal(result.items[0]?.historicalSignal, "below_recent_average");
});

test("searchProducts aborts slow Market API requests with client timeout", async () => {
